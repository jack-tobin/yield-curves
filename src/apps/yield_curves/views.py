...
@login_required
def get_bond_scatter_data(request, analysis_id, scatter_id):
    """Get bond data for a specific scatter."""
    analysis = get_object_or_404(Analysis, id=analysis_id, user=request.user)
    bond_scatter = get_object_or_404(BondScatter, id=scatter_id, analysis=analysis)
    ...
    # Calculate TTM and prepare data for chart
    chart_data = []
    today = dt.date.today()

    for metric in bond_metrics:
        ttm_days = (metric.bond.maturity_date - today).days
        ttm_years = ttm_days / 365.25  # Convert to years
...
@login_required
def get_all_scatters_data(request, analysis_id):
    """Get bond data for all scatters in an analysis."""
    analysis = get_object_or_404(Analysis, id=analysis_id, user=request.user)
    bond_scatters = analysis.bond_scatters.all()
